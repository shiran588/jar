...
function zywhm() {
    ...
    var setjson = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {}));
        ...
        url: $('hiker://empty#noRecordHistory#').rule(() => {
            ...
            d.push({
                ...
                url: $('hiker://empty').lazyRule(() => {
                    var md = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).ssmode;
                    if (md == 1) {
                        var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"ssmode\":\"1\"', '\"ssmode\":\"0\"');
                        writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                        back(true);
                        return 'toast://切换为搜索引擎列表单选模式成功！';
                    } else {
                        var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"ssmode\":\"0\"', '\"ssmode\":\"1\"');
                        writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                        back(true);
                ...
                url: "input://" + JSON.stringify({
                    ...
                    js: $.toString(() => {
                        ...
                        if (num == 'NaN' || num < 100) {
                            ...
                            var fileUrl = fetch('hiker://files/rules/timik/zywset.json', {}).replace(/"tmout":"[\d]*"/, '"tmout":"' + num + '"');
                            writeFile('hiker://files/rules/timik/zywset.json', fileUrl);
                            refreshPage(true);
                ...
                url: "input://" + JSON.stringify({
                    ...
                    js: $.toString(() => {
                        ...
                        if (num == 'NaN' || num < 1) {
                            ...
                            var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace(/"sscount":"[\d]*"/, '"sscount":"' + num + '"');
                            writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                            refreshPage(true);
            ...
            d.push({
                title: '打开编辑',
                url: $().lazyRule(() => {
                    return 'editFile://hiker://files/rules/timik/zywcj.txt'
                }),
                col_type: 'text_2'
            });
            d.push({
            ...
            d.push({
                ...
                url: $().lazyRule(() => {
                    var rulejs = request('https://gitlab.com/timiking/HikerRules/-/raw/master/zywcj/zywcj.js', {});
                    var parsejs = request('https://gitlab.com/timiking/HikerRules/-/raw/master/zywcj/parse.js', {});
                    eval(rulejs);
                    if (/CryptoJS/.test(parsejs)) {
            ...
            if(note==undefined){
                ...
                if (html.indexOf("</pic>") != -1&&listmod == '0') {
                    ...
                    items.push({
                        ...
                        desc: note,
                        url: arrr + "?ac="+(arrr.includes('jisubt')?'detail':'videolist')+"&ids=" + url + `@rule=js:eval(fetch('hiker://files/rules/timik/zywcj.js'));SSEJ();`,
                        col_type: "movie_2"
                    });
                } else {
...
function TWEJ() {
    ...
    var listmod = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).listmod;
    if (listmod == '1') {
        html = getResCode();
    } else {
                        ...
                        url: $(arrr + "?ac=list&pg=fypage&t=" + tyid).rule(() => {
                            ...
                            if (pn == '1') {
                                items.push({
                                    ...
                                    desc: '',
                                    url: $('hiker://empty').lazyRule(() => {
                                        var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"listmod\":\"0\"', '\"listmod\":\"1\"');
                                        writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                                        refreshPage();
                                        return 'toast://切换成功！'
                                    }),
                                ...
                                items.push({
                                    ...
                                    desc: '',
                                    url: $('hiker://empty').lazyRule(() => {
                                        var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"listmod\":\"1\"', '\"listmod\":\"0\"');
                                        writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                                        refreshPage();
                                        return 'toast://切换成功！'
                                    }),
...
function zywsea() {
    ...
    var ssmode = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).ssmode;
    //setError(skey)
    var src = fetch(getItem('zylink', 'hiker://files/rules/timik/zywcj.txt'), {});
        ...
        for (var l = 0; l < arrs.length; l++) {
            ...
            items.push({
                ...
                url: $('hiker://empty#noRecordHistory#$$$' + ss + '$$$fypage').rule((list,tabs) => {
                    ...
                    setPageTitle(tabs);
                    eval(fetch('hiker://files/rules/timik/zywcj.js'));
                    var timeou = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).tmout;
                    //获取搜索线程数量
                    var ssxc = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).sscount;
                    var skey = MY_URL.match(/\?wd\=(.*?)\&pg/)[1];
                    var ss = MY_URL.split('$$$')[1];
                                        ...
                                        for (var j = 0; j < list.length; j++) {
                                            ...
                                            if(note==undefined){
                                                ...
                                                items.push({
                                                    ...
                                                    url: param.ost.url + "?ac="+(param.ost.url.includes('jisubt')?'detail':'videolist')+"&ids=" + ids + `@rule=js:var erj=fetch("hiker://files/rules/timik/zywcj.js",{});eval(erj);SSEJ();`,
                                                    col_type: 'text_center_1'
                                                });
        ...
        for (var i = 0; i < arrs.length; i++) {
            ...
            for (var j = 0; j < list.length; j++) {
                ...
                if (!/itono|888hyk|okcj|openyun|8kvod/.test(list[j])) {
                    items.push({
                        title: list[j].split(',')[0],
                        url: list[j].split(',')[1].replace(/\n/, '') + ss.replace(/pg=\d*/g, 'pg=fypage') + `@rule=js:var erj=fetch("hiker://files/rules/timik/zywcj.js",{});eval(erj);zywerj();`,
                        col_type: 'text_4'
                    });
                }
            }
        ...
        for (var j = 0; j < list.length; j++) {
            ...
            if(note==undefined){
                ...
                items.push({
                    ...
                    desc: last + ' ·  ' + typ + ' ·  ' + dt,
                    url: domain + "?ac="+(domain.includes('jisubt')?'detail':'videolist')+"&ids=" + ids + `@rule=js:var erj=fetch("hiker://files/rules/timik/zywcj.js",{});eval(erj);SSEJ();`,
                    col_type: 'text_center_1'
                });
...
function SSEJ() {
    ...
    try {
        ...
        setPagePicUrl(pic);
            ...
            var dnen = JSON.parse(fetch('hiker://files/rules/timik/zywset.json', {})).enDn;
            items.push({
            	title:'断插：'+(dnen == 1?'““✅””':'❎'),
                url:$('hiker://empty#noRecordHistory#').lazyRule((dnen)=>{
                if (dnen == 1) {
                var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"enDn\":\"1\"', '\"enDn\":\"0\"');
                writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                refreshPage(false);
                return 'toast://已禁用断插调用！';
                } else {
                var fileUrl = fetch("hiker://files/rules/timik/zywset.json", {}).replace('\"enDn\":\"0\"', '\"enDn\":\"1\"');
                writeFile("hiker://files/rules/timik/zywset.json", fileUrl);
                refreshPage(false);
               ...
               if (list != null) {
                    ...
                    for (var j = 0; j < list.length; j++) {
                        ...
                        if (list.length <= 4) {
                            ...
                            items.push({
                                title: title,
                                url: 'hiker://empty##' + flag + '##' + url.replace(/\n*/g, '') + '##' +'#noPre#'+ `@lazyRule=.js:/*refreshX5WebView*/eval(fetch('hiker://files/rules/timik/zywcj.js'));lazyRu();`,
...
for (var i in realurl) {
    if (src.search(realurl[i]) != -1) {
        if (dnen == '1') {
            ...
            danmufun(src)
            try {
            var fileUrl=fetchCache("https://gitlab.com/timiking/HikerRules/-/raw/master/zywcj/parse.js", 24);
            } catch (e) {
...
//预处理代码
function zywpre() {
    if (!fetch('hiker://files/rules/timik/zywset.json', {})) {
        var set = `{"ssmode":"0","listmod":"0","sscount":"5","tmout":"3000","enDn":"0"}`;
        writeFile("hiker://files/rules/timik/zywset.json", set);
    }
    var ruleset = fetch('hiker://files/rules/timik/zywset.json', {});
